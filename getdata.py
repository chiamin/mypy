...
def special_analysis (filename, prefix):
  if os.path.exists (filename):
    ...
    for oi in obs3:
      binone.run (oi[0], oi[1-len(oi)/divide:],NBmin=BINNUM)
...
for job in readfile((DATDIR+'/'+JOBLIST).rstrip())[0]:
  ...
  for raw in raws:
    for rawi in raw:
      binone = BinAnaly()
      binone.run (rawi[0], rawi[1-len(rawi)/divide:],NBmin=BINNUM)
      binone.write (datfile)
  # Obs3
  special_analysis (RAWDIR+'/'+job+'_open.obs3', 'ZN_open')
  special_analysis (RAWDIR+'/'+job+'_close.obs3', 'ZN_close')
