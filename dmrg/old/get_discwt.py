...
# returned dat: error energy error_per_site m index
  out,err = sp.Popen(['discwt2.p',filename], stdout=sp.PIPE).communicate()
  dat = [map(float,list(di.split())) for di in out.split("\n")]
  dat.remove([])
  return dat

def drop_small_m (dat, mlowerlim=0, mupperlim=1000000000):
