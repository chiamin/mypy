...
for name in names:
  ...
  for ni in range(1,JOBN+1):
    ...
    filename3 = str(ni)+'/rawdata/'+name+'_open.obs3'
    dat3o.extend (readfile (filename3, raw=True))
    filename3 = str(ni)+'/rawdata/'+name+'_close.obs3'
    dat3c.extend (readfile (filename3, raw=True))
  writefile ('rawdata/'+name+'.obs', dat, raw=True)
  writefile ('rawdata/'+name+'_open.obs3', dat3o, raw=True)
  writefile ('rawdata/'+name+'_close.obs3', dat3c, raw=True)
